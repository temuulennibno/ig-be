...
router.post("/", authMiddleware, async (req, res) => {
  ...
  const post = await PostModel.create({ _id: nanoid(), description, imageUrl, createdBy: req.user._id });
...
router.post("/:postId/like", authMiddleware, async (req, res) => {
  ...
  const existingLike = await PostLikeModel.findOne({ post: postId, createdBy: req.user._id });
  ...
  if (!existingLike) {
    ...
    return res.status(200).send({ message: "Амжилттай лайк дарлаа", isLiked: true });
  }

  await PostLikeModel.findOneAndDelete(existingLike._id);
  return res.status(200).send({ message: "Амжилттай лайкаа буцаалаа", isLiked: false });
});
